...
#'
#'@description mlmm function handles Bayesian multilevel model with responses
#that is not-missing-at-random. It is motivated from analysing mass spectrometry
#data with respondent dependant non-missing-at-random missingness, 
...
#'which is used to define second level unit such as subject and explanatory variables.
#'
...
#'the indicator variable for second level unit such as subject or a sampling unit, 
#'an indicator for missingness and indictor of censoring.  
#'Missingness and censor are two different classification, there should not have any
#'overlap between missingness and censored. 
...
{ 
    ...
    if (respond_dep==1) 
    parsstr=c("U","beta2","alpha","alpha_response") else parsstr=c("U","beta2","alpha")
    ...
    if (usefit==TRUE){
        ...
        if (savefile==TRUE) 
        fitmlmm=rstan::stan(file=file.path(mlmm_path,"mlmm/exec/mmlm_code.stan"),
    ...
    print(fitmlmm)
    if (savefile==TRUE) 
    utils::write.csv(as.array(fitmlmm),file=file.path(getwd(),"outsummary.csv"),row.names=TRUE)
    return(fitmlmm)
}
