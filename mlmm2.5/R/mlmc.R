...
#' @description mlmc() handles Bayesian multilevel model with responses that is left-censored
#'  and not-missing-at-random. It is motivated from analysing mass-spectrometry data that 
#'  has missing values non-missing-at-random, and there are known covariates associating
#'  with the missingness. It also imputes values under a known detectable limit in the response values.
#' @import methods MASS Matrix Rcpp stats4 ggplot2 
#'
#' @param formula_completed The main regression model formula; It has the same formula format as lmr() and 
#'        it is used to define the first level response and its explanatory variables.
...
#' @param pdata The dataset contains response and predictors in a long format. Response is a vector with an 
#'        indictor variable to define the corresponding unit. The data needs to have the following rudimental 
#'        variables: the indicator variable for first level response, second level indicator variable for subject, 
#'        or a sampling unit, an indicator for missingness and indictor of censoring. Missingness and censor are 
#'        two different classification, there should not have any overlap between missingness and censored. 
...
#'
#'
#' @return Return of the function is the result fitted by stan. It will have the summarized parameters from 
#'        all chains and summary results for each chain.
...
#' geneid=rep(seq_len(50),16);sid=c(rep(seq_len(25),16),rep(seq_len(50)+25,16))
...
#' for ( i in seq_len(800)) {if ((pdata$miss[i]==1) & (pdata$censor[i]==1)) pdata$miss[i]=0};
#' for ( i in seq_len(800)) {if (pdata$miss[i]==1) pdata$var1[i]=NA;
...
mlmc=function(formula_completed,formula_missing,formula_censor=NULL,formula_subject,
pdata,respond_dep_missing=TRUE,response_censorlim=NULL,pidname,sidname,iterno=100,
chains=3,thin=1,seed=125,algorithm="NUTS",warmup=floor(iterno/2),adapt_delta_value=0.85,
savefile=FALSE,usefit=FALSE)
{
    ...
    {tt4=stats::terms(formula_censor);mf4=stats::model.frame(tt4,pdata,na.action='na.pass');
    mm4=stats::model.matrix(mf4,pdata);censor=stats::model.response(mf4)} else ncensor=0;
    if (!is.null(response_censorlim)) censor_lim=response_censorlim else stop("No response censor limit defined");
    ...
    if ((!is.null(formula_censor)))
    {prstan_data=list(nobs=nobs, nmiss=nmiss, ncensor=ncensor, nsid=nsid, np=np,
    npred=npred, npred_miss=npred_miss, npred_sub=npred_sub,
    censor_lim=censor_lim, respond_dep=respond_dep, y=y_all[(missing!=1 & censor!=1)],
    sid=sid, pid=pid, pred=pred, pred_miss=pred_miss, pred_sub=pred_sub, miss_obs=miss_obs,
    ...
    {prstan_data=list(nobs=nobs, nmiss=nmiss, ncensor=ncensor, nsid=nsid, np=np,
    npred=npred, npred_miss=npred_miss, npred_sub=npred_sub, censor_lim=censor_lim,
    respond_dep=respond_dep, y=y_all[(missing!=1 & censor!=1),], sid=sid, pid=pid,
    pred=pred, pred_miss=pred_miss, pred_sub=pred_sub, miss_obs=miss_obs, sid_m=sid_m, 
    pid_m=pid_m, pred_m=pred_m, pred_miss_m=pred_miss_m, pred_sub_m=pred_sub_m, miss_m=miss_m,
    ...
    if (usefit==TRUE) 
        ...
        if (savefile==TRUE)  
		fitmlmc=rstan::sampling(stanfit,data=prstan_data,iter=iterno,init=initvalue1,pars=parsstr,seed=seed,thin=thin,
        algorithm=algorithm,warmup=warmup,chains=chains,control=list(adapt_delta=adapt_delta_value),sample_file=file.path(getwd(),"samples")) else 
		{fitmlmc=rstan::sampling(stanfit,data=prstan_data,iter=iterno,init=initvalue1,pars=parsstr,seed=seed,thin=thin,
        algorithm=algorithm,warmup=warmup,chains=chains,control=list(adapt_delta=adapt_delta_value),sample_file=NULL)}
        }  else  
        {if (savefile==TRUE) 
        fitmlmc=rstan::stan(file=file.path(mlmm_path,"mlmm/exec/mlmc_code.stan"),data=prstan_data,iter=iterno,init=initvalue1,pars=parsstr,
        seed=seed,thin=thin,algorithm=algorithm,warmup=warmup,chains=chains,control=list(adapt_delta=adapt_delta_value),
        sample_file=file.path(getwd(),"samples"),save_dso=TRUE) else 
        {fitmlmc=rstan::stan(file=file.path(mlmm_path,"mlmm/exec/mlmc_code.stan"),data=prstan_data,iter=iterno,init=initvalue1,pars=parsstr,
        seed=seed,thin=thin,algorithm=algorithm,warmup=warmup,chains=chains,control=list(adapt_delta=adapt_delta_value),sample_file=NULL,save_dso=TRUE)}
        }

    print(fitmlmc);plot(fitmlmc)
    if (savefile==TRUE) utils::write.csv(as.array(fitmlmc),file=file.path(getwd(),"outsummary.csv"),row.names=TRUE)
    return(fitmlmc)
}
